#language: es
@PPB002-CAP_F_082 and @skip and @KO

Característica: PPB002-CAP_F_082 - Visualizacion detalle Galeria
  Como un usuario comun
  Quiero acceder al portal
  Para visualizar detalle galeria

  Escenario: Cargando el Portal de Referencia
    Dado PPB002-CAPF082 - Cargando el Navegador
    Cuando PPB002-CAPF082 - Se haya cargado la web
    Y PPB002-CAPF082 - Aceptar las cookies
    Y PPB002-CAPF082 - Mostrar sin fecha
    Y PPB002-CAPF082 - Mostrar sin subtitulo
    Y PPB002-CAPF082 - Mostrar sin texto
    Y PPB002-CAPF082 - Mostrar sin texto miniatura
    Y PPB002-CAPF082 - Mostrar sin direccion
    Y PPB002-CAPF082 - Mostrar sin documento
    Y PPB002-CAPF082 - Mostrar sin contenido web
    Y PPB002-CAPF082 - Añadir documento multimedia
    Y PPB002-CAPF082 - Añadir contenido web

    








