package org.medioAmbiente.PPB001.CAP_F_114.page;
...
public class MyPage extends PageObject {
    ...
    @FindBy(xpath = "//input[@id=\"_previewContenidoWeb_userName\"]")
    private WebElementFacade Usuario_Creador;

    @FindBy(xpath = "//button[contains(text(),'Buscar')]")
    private WebElementFacade btnBuscar;

    @FindBy(xpath = "(//td[4])[1]")
    private WebElementFacade Validar_Estado;

    @FindBy(xpath = "(//td[2])[1]")
    private WebElementFacade Validar_Creador;
    ...
    public void seleccionarTodoCorrespondiencia() throws InterruptedException {
        Select Correspondencia = new Select(getDriver().findElement(By.id("_previewContenidoWeb_andOperator")));
        Correspondencia.selectByVisibleText("Todos");
    }

    public void buscarPorUsuarioCreador() throws InterruptedException {
        Usuario_Creador.sendKeys("Admin CMAOT");
    }
    ...
    public void validarBusqueda() throws InterruptedException {
        String Estado = Validar_Estado.getText();
        String Creador = Validar_Creador.getText();
        if(Estado.equals("Borrador") && Creador.equals("Admin CMAOT")) {
            Assert.assertTrue(true);
        }else{
            Assert.fail("Lista no valida");
        }
    }
}
