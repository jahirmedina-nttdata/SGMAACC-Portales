package org.medioAmbiente.PPB005.CAP_F_001_05.page;
...
public class MyPage extends PageObject {
    ...
    @FindBy(xpath = "//span[contains(text(),\"cmaot_testing\")]")
    private WebElementFacade Cmaot;

    @FindBy(xpath = "//a[@data-title=\"Mis organizaciones\"]")
    private WebElementFacade Organizaciones;
    ...
    public void accederAlPortal() throws InterruptedException, URISyntaxException {
        getDriver().get(new URI(getDriver().getCurrentUrl()).resolve("/medioambiente/portal/web/portal-conocimiento").toString());
    }

    public void clickarMenuLateral() throws InterruptedException{
        ...
        if (lista.size() != 0) {
            Cmaot.waitUntilClickable();
            Cmaot.click();
            Organizaciones.click();
        } else {
            Menu_Vertical.click();
            Cmaot.waitUntilClickable();
            Cmaot.click();
